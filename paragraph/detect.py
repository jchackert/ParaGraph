...
def _load_paragraphignore(root: Path) -> list[tuple[Path, str]]:
    """Read .paragraphignore from root **and ancestor directories**.
    ...
    the .paragraphignore file was found — so patterns written relative to a
...
def _is_ignored(path: Path, root: Path, patterns: list[tuple[Path, str]]) -> bool:
    """Return True if path matches any .paragraphignore pattern."""
    if not patterns:
        return False
    ...
    for anchor, pattern in patterns:
        ...
        try:
            ...
            pass
        # Also try relative to the anchor dir (the .paragraphignore's location),
...
def detect(root: Path, *, follow_symlinks: bool = False) -> dict:
    ...
    ignore_patterns = _load_paragraphignore(root)
        ...
        if ftype:
            ...
            if p.suffix.lower() in OFFICE_EXTENSIONS:
                ...
                if md_path:
                    ...
                    skipped_sensitive.append(str(p) + " [office conversion failed - pip install paragraph[office]]")
                continue
    ...
    return {
        ...
        "paragraphignore_patterns": len(ignore_patterns),
    }
