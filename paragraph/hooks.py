...
# Installed by: paragraph hook install
...
print(f'[paragraph hook] {len(changed)} file(s) changed - rebuilding graph...')

try:
    from paragraph.watch import _rebuild_code
    _rebuild_code(Path('.'))
except Exception as exc:
    print(f'[paragraph hook] Rebuild failed: {exc}')
    sys.exit(1)
"
# graphify-hook-end
"""
...
try:
    ...
    print(f'[paragraph] Rebuild failed: {exc}')
    sys.exit(1)
"
...
def _uninstall_hook(hooks_dir: Path, name: str, marker: str, marker_end: str) -> str:
    ...
    if marker not in content:
        return f"paragraph hook not found in {name} - nothing to remove."
    new_content = re.sub(
...
def status(path: Path = Path(".")) -> str:
    """Check if paragraph hooks are installed."""
    root = _git_root(path)
    if root is None:
