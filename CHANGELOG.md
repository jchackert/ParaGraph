...
- **Fix: semantic cache entries no longer mask AST extraction** -- AST and semantic (LLM) results shared one cache key (`{hash}.json`), so any file that ever received a semantic fragment silently lost its entire structural extraction on every `paragraph update` (classes, methods, calls — everything; the fragment was returned as the file's extraction). AST entries now live in their own `{hash}.ast.json` namespace
